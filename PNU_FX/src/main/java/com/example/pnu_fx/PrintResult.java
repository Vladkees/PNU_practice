...
public class PrintResult {
    ...
    protected void onUploadButtonClick() {
        ...
        if (selectedFile1 != null && selectedFile2 != null) {
            ...
            // Розбиваємо текст на абзаци для виведення результатів
            ...
            // Викликаємо нову функцію порівняння абзаців
    ...
    private void compareParagraphs(List<String> firstParagraphs, List<String> secondParagraphs) {
        ...
        int significantChangeThreshold = 10;  // Поріг для значних змін
        LevensteinDistance levenstein = new LevensteinDistance();
        int totalDistance = 0;  // Для підсумовування загальної відстані

        // Для кожного абзацу з першого тексту
        for (int i = 0; i < firstParagraphs.size(); i++) {
            String firstParagraph = firstParagraphs.get(i);
            if (firstParagraph.trim().isEmpty()) continue;  // Пропускаємо порожні абзаци

            int maxMatches = 0;
            String bestMatchParagraph = null;

            // Порівнюємо поточний абзац з усіма абзацами другого тексту
            for (int j = 0; j < secondParagraphs.size(); j++) {
                String secondParagraph = secondParagraphs.get(j);
                if (secondParagraph.trim().isEmpty()) continue;  // Пропускаємо порожні абзаци

                // Визначаємо кількість збігів між абзацами
            ...
            // Якщо знайдений найбільший збіг, порівнюємо цей абзац
    ...
    // Функція для підрахунку кількості збігів між двома абзацами
    private int countMatches(String s1, String s2) {
        int matchCount = 0;
        // Перевірка, скільки символів збігаються між двома абзацами
